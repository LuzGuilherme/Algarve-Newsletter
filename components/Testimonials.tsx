...
const Testimonials: React.FC = () => {
  return (
    <>
      ...
      <section className="bg-[#004E55] py-32 px-4 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          ...
          <div className="grid md:grid-cols-3 gap-8 mb-12">
            {TESTIMONIALS.map((t, idx) => (
              <div
                key={t.id}
                className={`
                  group relative p-8 rounded-[40px] flex flex-col h-full 
              ...
              >
                ...
                <div className="flex gap-1 mb-6">
                  {[...Array(5)].map((_, i) => (
                    <Star
                      key={i}
                      className={`w-5 h-5 ${i < Math.floor(t.rating) ? 'fill-amber-400 text-amber-400' : 'text-white/20'}`}
                    />
                  ))}
                </div>

                {/* Content */}
                <p className="text-white/90 text-lg leading-relaxed mb-8 flex-grow font-medium relative z-10">
                  "{t.content}"
                </p>
                ...
                <div className="flex items-center gap-4 border-t border-white/10 pt-6 mt-auto">
                  <div className="relative">
                    <img src={t.avatar} alt={t.name} className="w-14 h-14 rounded-full object-cover border-2 border-white/20 group-hover:border-emerald-400/50 transition-colors" />
                  ...
                  <div>
                    <h4 className="font-bold text-white text-lg tracking-tight group-hover:text-emerald-300 transition-colors">{t.name}</h4>
                    <p className="text-xs font-bold text-white/50 uppercase tracking-widest">{t.role}</p>
                  </div>
                </div>
              </div>
            ))}
          ...
          <div className="flex flex-wrap justify-center gap-8 md:gap-16 border-t border-white/10 pt-12">
            ...
            <div className="text-center">
              ...
              <div className="text-sm text-white/50 uppercase tracking-widest font-bold">Average Rating</div>
            </div>
            <div className="text-center">
              <div className="text-3xl font-black text-white mb-1">100+</div>
              <div className="text-sm text-white/50 uppercase tracking-widest font-bold">Hidden Gems</div>
            </div>
          </div>
